...
add_action('init', function () {
    ...
    pll_register_string('sport_hero_suffix', '& Thousands of Live Sport Events with iBostreaming', $group);
    ...
    pll_register_string('sport_hero_fallback_subtitle', 'Stream <strong>%s</strong> live with iBostreaming. No cable required. <strong>No blackouts. No restrictions.</strong>', $group, true);
    ...
    pll_register_string('sport_hero_disclaimer', 'No contract • Cancel anytime • 7-day money-back', $group);
    ...
    // ── Features (6 cards) ────────────────────────────────────────────────────
    pll_register_string('sport_feat_tag', 'Why iBostreaming', $group);
    pll_register_string('sport_feat_title', 'Watch %s with <span class="gradient-text">iBostreaming</span>', $group, true);
    ...
    pll_register_string('sport_price_guarantee', '7-day money-back guarantee.', $group);
    ...
    pll_register_string('sport_price_trust_3_desc', '7-day money-back guarantee.', $group);
    ...
    pll_register_string('sport_faq_subtitle', 'Common questions about watching %s with iBostreaming', $group);

    // ── FAQ default Q&A (used when no ACF repeater items are set) ────────────
    pll_register_string('sport_faq_q_1', 'How do I watch %s with iBostreaming?', $group);
    pll_register_string('sport_faq_a_1', 'Simply subscribe to any iBostreaming plan, download our app on your device, and search for %s. You\'ll be streaming in minutes.', $group);
    ...
    pll_register_string('sport_faq_q_5', 'Is there a free trial?', $group);
    pll_register_string('sport_faq_a_5', 'We do not offer a free trial. Instead every plan carries a 7-day money-back guarantee, so you can test the full service and get your money back if it does not work for you.', $group);
    pll_register_string('sport_faq_q_6', 'Do I need a cable subscription?', $group);
    pll_register_string('sport_faq_a_6', 'No! iBostreaming is a standalone streaming service. No cable, no satellite dish, no contracts. Just an internet connection.', $group);

    // ── CTA ──────────────────────────────────────────────────────────────────
    pll_register_string('sport_cta_title', 'Ready to Watch %s?', $group);
    pll_register_string('sport_cta_subtitle', 'Join thousands of satisfied customers streaming %s and 40,000+ channels with iBostreaming.', $group);
