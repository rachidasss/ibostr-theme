...
class IPTV_Currency_Settings
{
    ...
    //
    // These ARE the live prices, not a placeholder: WooCommerce is no longer
    // installed, so wc_get_product_id_by_sku() does not exist and
    // calculate_all_prices() falls through to this array for every SKU. Whatever
    // is written here is what visitors are quoted.
    //
    ...
    private $default_usd = array(
        ...
        '3_months' => array('1_device' => 24.99, '2_devices' => 39.99, '3_devices' => 45.99, '4_devices' => 75.99),
        '6_months' => array('1_device' => 34.99, '2_devices' => 64.99, '3_devices' => 75.99, '4_devices' => 129.99),
        '12_months' => array('1_device' => 59.99, '2_devices' => 109.99, '3_devices' => 149.99, '4_devices' => 189.99),
    );
    ...
    public static function get_price_table()
    {
        ...
        if (is_array($stored)
            ...
            && $stored['fingerprint'] === self::price_source_fingerprint()
        ) {
            return $stored['prices'];
        }

        // Either nothing has ever been built, or the inputs changed since it was.
        // Rebuild rather than serve a stale table — see price_source_fingerprint().
        return self::rebuild_price_table(false);
    }

    /**
     * Fingerprint of everything the table is derived from.
     *
     * The stored table used to be trusted for as long as it existed, and the only
     * things that invalidated it were admin-side hooks (saving a price, saving a
     * product). That breaks when the prices move in *code* rather than in the
     * database: with WooCommerce uninstalled, calculate_all_prices() falls back to
     * $default_usd, so editing that array is a price change with no hook behind
     * it — and every visitor kept being quoted the numbers cached before the edit.
     *
     * Hashing the source inputs makes the cache self-healing: change the defaults
     * or the conversion rates and the next request rebuilds on its own.
     */
    private static function price_source_fingerprint()
    {
        $instance = self::instance();

        return md5(serialize(array(
            'usd'   => $instance->default_usd,
            'rates' => get_option('iptv_conversion_rates', array()),
        )));
    }
    ...
    public static function rebuild_price_table($purge = true)
    {
        ...
        update_option(
            ...
            array(
                'prices'       => $prices,
                'rates'        => get_option('iptv_conversion_rates', array()),
                'fingerprint'  => self::price_source_fingerprint(),
                'generated_at' => time(),
            ),
